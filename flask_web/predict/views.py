...
def SelectAlgorithm(algorithmtype):
    job = joblib.load('../static/model/RandomForest_Regression_Morgan.pkl')
    if algorithmtype ==0:
        job = joblib.load('../static/model/RandomForest_Regression_Morgan.pkl')
    elif algorithmtype ==1:
        job = joblib.load('../static/model/SVM_Regression_Hybridization.pkl')
    elif algorithmtype == 2:
        job = joblib.load('../static/model/gbdt.joblib')
    return job
